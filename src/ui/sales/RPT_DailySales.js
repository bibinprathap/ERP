...
export default class RPT_DailySales extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_getdailysalesreport',constants.COMPANY) +   ';@dates|\'' + yyyymmdd + '\';@Br|\'' + constants.ERP_BRANCH + '\''
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTDAILYSALES&RefNo=' + RefNo + '&heading=Daily Sales Report - ' + ddmmyyyy + '&subheading={BRANCH}&Company=' + constants.COMPANY
    ...
    viewReport (event) {
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/getdailysalesreport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "CUSTSUP", name: "Customer", field: "CUSTSUP", width: 150, groupTotalsFormatter: sumTotalLabel });
    ...
    resetOptions={this.resetOptions} reportTitle='Daily Sales Report'/> }
