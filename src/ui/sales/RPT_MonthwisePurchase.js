import RPT_MonthwisePurchase_Form from './RPT_MonthwisePurchase_Form';
...
export default class RPT_MonthwisePurchase extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_GetMonthWisePurchaseReport',constants.COMPANY) +   ';@fromdates|' + yyyymmddf + ';@todates|' + yyyymmddt + ';@br|' + constants.getFieldValue( this.RPT_DailyOpOptions.branch) + ';@sman|' + constants.getFieldValue(this.RPT_DailyOpOptions.salesman) + ';@custorsup|' + constants.getFieldValue(this.RPT_DailyOpOptions.custsupno);
        var reportURL=constants.DSREPORTURL + '/ReportPrint.aspx?ReportCode=MONTH_PUR_REPORT&RefNo=' + RefNo + '&heading=' + ddmmyyyyfrom  +' - ' + ddmmyyyyto + '&subheading={BRANCH}&subheading1=\'' + mSub1 + '\'&Company=' + constants.COMPANY;
    ...
    viewReport (event) {
       ...
       if(constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)) 
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/monthwisepurchasereport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "date", name: "Date", field: "yyyymm", width: 60,cssClass: "cell-title" });
    ...
    render () {
                return(
                        <div>   
                            {  
                                <RPT_MonthwisePurchase_Form RPT_DailyOpOptions={this.RPT_DailyOpOptions} 
                                    viewReport = {this.viewReport} 
                                    printReport={this.printReport} 
                                    resetOptions={this.resetOptions} 
                                    reportTitle='Monthwise Purchase Report'/> 
                            }
