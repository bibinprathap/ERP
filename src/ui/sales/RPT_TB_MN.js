import RPT_TB_MN_Form from './RPT_TB_MN_Form';
...
export default class RPT_TB_MN extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {   
        var hd;
        hd='Year : ' + constants.getFieldValue(this.RPT_TBOptions.filtervalue)  + ', Month : ' + constants.getFieldValue(this.RPT_TBOptions.custsupname) 
        
        var RefNo='PROC|' + constants.getprocedurename('SP_GetTB',constants.COMPANY) +   ';@Tp|\'M\';@summary|\'' + constants.getFieldValue(this.RPT_TBOptions.filtertype)+ '\';@LedgerOnly|\'' +  constants.getFieldValue( this.RPT_TBOptions.custsupno) +  '\';@dtFrom|\'' + constants.getFieldValue(this.RPT_TBOptions.filtervalue) + '\';@dtTo|\'' + constants.getFieldValue(this.RPT_TBOptions.custsupname) + '\';@br|\'' + constants.getFieldValue(this.RPT_TBOptions.branch) + '\''; 
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTTB_MN&RefNo=' + RefNo + '&heading=' + hd + '&subheading={BRANCH}&Company=' + constants.COMPANY
    ...
    viewReport (event) {
        window.setcompany();
        var loadrequest = {parem1:'M', parem2:constants.getFieldValue(this.RPT_TBOptions.filtertype), parem3:constants.getFieldValue( this.RPT_TBOptions.custsupno), parem4:constants.getFieldValue(this.RPT_TBOptions.filtervalue), parem5:constants.getFieldValue(this.RPT_TBOptions.custsupname) , parem6:constants.getFieldValue(this.RPT_TBOptions.branch), company:constants.COMPANY};
        var dailyreportdata = this.RPT_TBOptions;
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/get_tb_mn',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                var columns = [
                  ...
                  { id: "amt3", name: "Prev Month Debit", field: "amt3", minWidth: 120, formatter:numformatter1 },
                  { id: "amt4", name: "Prev Month Credit", field: "amt4", minWidth: 120 , formatter:numformatter1},
                  { id: "amt1", name: "Cur Month Debit", field: "amt1", minWidth: 120, formatter:numformatter1 },
                  { id: "amt2", name: "Cur Month Credit", field: "amt2", minWidth: 120 , formatter:numformatter1}
                ];
...
render () {
    ...
    <div>   
        ...
        {  <RPT_TB_MN_Form RPT_TBOptions={this.RPT_TBOptions} 
            viewReport = {this.viewReport} printReport={this.printReport}  resetOptions={this.resetOptions} reportTitle='Trail Balance (Monthly)'/> }
