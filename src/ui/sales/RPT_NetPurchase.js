import RPT_NetPurchase_Form from './RPT_NetPurchase_Form';
...
export default class RPT_NetPurchase extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_GetNetPurchaseReport',constants.COMPANY) +   ';@fromdates|' + yyyymmddf + ';@todates|' + yyyymmddt + ';@br|' + constants.getFieldValue( this.RPT_DailyOpOptions.branch) + ';@sman|' + constants.getFieldValue(this.RPT_DailyOpOptions.salesman) + ';@custorsup|' + constants.getFieldValue(this.RPT_DailyOpOptions.custsupno);
        var reportURL=constants.DSREPORTURL + '/ReportPrint.aspx?ReportCode=NET_PUR_REPORT&RefNo=' + RefNo + '&heading=' + ddmmyyyyfrom  +' - ' + ddmmyyyyto + '&subheading={BRANCH}&subheading1=\'' + mSub1 + '\'&Company=' + constants.COMPANY;
    ...
    viewReport (event) {
       ...
       if(constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)) 
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/netpurchasereport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "docdate", name: "Date", field: "docdate", width:100});
                columns.push({ id: "docno", name: "Doc.No", field: "docno", width:70});
                columns.push({ id: "gross", name: "Gross", field: "gross", width:70});
                columns.push({ id: "amount", name: "Amount", field: "amount", width: 50 ,formatter:numformatter,  groupTotalsFormatter: sumTotalsFormatter });
                columns.push({ id: "netpurchase", name: "Net Purchae", field: "netpurchase", width: 70 ,formatter:numformatter,  groupTotalsFormatter: sumTotalsFormatter });
                columns.push({ id: "supplier", name: "Supplier", field: "Customer", width: 150});
    ...
    render () {
        ...
        <div>   
            ...
            {  <RPT_NetPurchase_Form RPT_DailyOpOptions={this.RPT_DailyOpOptions} 
                    viewReport = {this.viewReport} printReport={this.printReport} 
                    resetOptions={this.resetOptions} reportTitle='Net Purchase Report'/> }
