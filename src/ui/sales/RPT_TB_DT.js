import RPT_TB_DT_Form from './RPT_TB_DT_Form';
...
export default class RPT_TB_DT extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {   
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_GetTB',constants.COMPANY) +   ';@Tp|\'T\';@summary|\'' + constants.getFieldValue(this.RPT_TBOptions.filtertype) + '\';@LedgerOnly|\'' + constants.getFieldValue( this.RPT_TBOptions.custsupno) +  '\';@dtFrom|\'' + fromdate + '\';@dtTo|\'' + todate + '\';@br|\'' + constants.getFieldValue(this.RPT_TBOptions.branch) + '\'';
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTTB_DT&RefNo=' + RefNo + '&heading=' + hd + '&subheading={BRANCH}&Company=' + constants.COMPANY
    ...
    viewReport (event) {
        ...
        fromdate =''
        window.setcompany();
        var loadrequest = {parem1:'T', parem2:constants.getFieldValue(this.RPT_TBOptions.filtertype), parem3:constants.getFieldValue(this.RPT_TBOptions.custsupno)  , parem4:fromdate, parem5:todate, parem6:constants.getFieldValue(this.RPT_TBOptions.branch), company:constants.COMPANY};
        var dailyreportdata = this.RPT_TBOptions;
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/get_tb_dt',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                var columns = [
                  ...
                  { id: "amt1", name: "Debit", field: "amt1", minWidth: 150, formatter:numformatter1 },
                  { id: "amt2", name: "Credit", field: "amt2", minWidth: 150 , formatter:numformatter1}
                ];
...
render () {
    ...
    <div>   
        ...
        {  <RPT_TB_DT_Form RPT_TBOptions={this.RPT_TBOptions} 
            viewReport = {this.viewReport} printReport={this.printReport} 
resetOptions={this.resetOptions} reportTitle='Trail Balance (as on Date)'/> }
