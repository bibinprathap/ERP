...
export default class RPT_DailyOp extends React.Component 
{
    ...
    printReport (event) {   
        ...
        var RefNo='PROC|' + constants.getprocedurename('procgetdailyopertion',constants.COMPANY) +   ';@dates|\'' + yyyymmdd + '\';@Br|\'' + constants.ERP_BRANCH + '\';@Rpt|\'Y\''
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTDAILYOP&RefNo=' + RefNo + '&heading=' + ddmmyyyy + '&subheading={BRANCH}&Company=' + constants.COMPANY
    ...
    viewReport (event) {
        ...
        //    constants.ERP_BRANCH = branch;
         ...
         var dailyreportdata = this.RPT_DailyOpOptions;
                
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/getdailyopreport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                var grid;
                var data = respoce.dailyopertion;
                //dailyreportdata.cashamount = respoce.dailycash[0].totalcash;
                dailyreportdata.cashamount = respoce.dailycash[0].clbal;
                var dataView;
                var columns = [];
                ...
                columns.push({ id: "id", name: "SNo", field: "sno", width: 40, cssClass: "cell-title" });
                ...
                columns.push({ id: "amount", name: "Net Amount", field: "amount",  groupTotalsFormatter: sumTotalsFormatter, width: 90, formatter:numformatter } );
                ...
                columns.push({ id: "USERNAME", name: "User Name", field: "USERNAME", minWidth: 30 } );
                columns.push({ id: "vdate", name: "Voucher Date", field: "vdate", minWidth: 100 } );
                ...
                setTimeout(function(){
                    ...
                    grid.onClick.subscribe(function (e, args) {
                        var item = dataView.getItem(args.row);
                            window.CashSalespopup = Popup.show(document.getElementById('itemquirypopup'), Screen.getBody(), 'top right', { 'constrainToScreen': true, autoHide: false, width: 892, height: 677, style: { 'border': '3px solid black', 'backgroundColor': '#0088ce', 'z-index': '9999' } });
                            ReactDOM.render(<VoucherEntries ispopup = {true} br={constants.ERP_BRANCH} docno= {item.docno}  cashsalesdocno ={item.vicno} />, document.getElementById('myiteminquiry')); 
                setTimeout(function() {
                    window.VoucherEntriesFormref.getvouchedetails(null,item.docno.split('-')[1],13) 
                }.bind(this), 200);  
                  });
                    ...
                    dataView.setGrouping({
                        ...
                        formatter: function (g) {
                            return "Document Type:  " + constants.doctypename(g.value) + "  <span style='color:green'>(" + g.count + " items)</span>";
                        },
                        aggregators: [
    ...
    resetOptions={this.resetOptions} reportTitle='Daily Operation Report'/> }
