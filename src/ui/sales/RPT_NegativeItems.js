import RPT_NegativeItems_Form from './RPT_NegativeItems_Form';
...
export default class RPT_NegativeItems extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        if(constants.getFieldValue( this.RPT_DailyOpOptions.branch)) 
            br = constants.getFieldValue( this.RPT_DailyOpOptions.branch)
        if(constants.getFieldValue( this.RPT_DailyOpOptions.salesman)) 
            sale = constants.getFieldValue( this.RPT_DailyOpOptions.salesman)
        if(constants.getFieldValue( this.RPT_DailyOpOptions.custsupno)) 
            cust = constants.getFieldValue( this.RPT_DailyOpOptions.custsupno)
        var RefNo='PROC|' + constants.getprocedurename('SP_GetNegativeItemsList',constants.COMPANY) +   ';@fromdates|' + yyyymmddf + ';@todates|' + yyyymmddt + ';@br|' + br + ';@sman|' + sale + ';@custorsup|' + cust;
        var reportURL=constants.DSREPORTURL + '/ReportPrint.aspx?ReportCode=NGTV_ITEM_LIST&RefNo=' + RefNo + '&heading=' + ddmmyyyyfrom  +' - ' + ddmmyyyyto + '&subheading={BRANCH}&subheading1=\'' + mSub1 + '\'&Company=' + constants.COMPANY;
    ...
    viewReport (event) {
       ...
       if(constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)) 
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/negativeitemsreport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "id", name: "S.No", field: "id", width: 40});
                columns.push({ id: "br", name: "Br", field: "Branch", width: 60});
                columns.push({ id: "brand", name: "Brand", field: "brand", width: 70 } );
                columns.push({ id: "Origin", name: "Origin", field: "Origin", width: 70 } );
                columns.push({ id: "idesc", name: "Item Name", field: "idesc", width: 150} );
                columns.push({ id: "qty", name: "Qty", field: "QTY1", width: 50 });
                columns.push({ id: "PartNo", name: "PartNo", field: "PartNo", width: 180});              
    ...
    render () {
        ...
        <div>   
            ...
            {  <RPT_NegativeItems_Form RPT_DailyOpOptions={this.RPT_DailyOpOptions} 
                    viewReport = {this.viewReport} printReport={this.printReport} 
                    resetOptions={this.resetOptions} reportTitle='Negative Items Report'/> }
