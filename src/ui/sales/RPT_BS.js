...
export default class RPT_BS extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {   
        ...
        var hd;
        hd='As on ' + todated        
        var RefNo='PROC|' + constants.getprocedurename('SP_GetBS',constants.COMPANY) +   ';@dtTo|\'' + todate + '\'';
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTBS&RefNo=' + RefNo + '&heading=' + hd + '&Company=' + constants.COMPANY
    ...
    printReportPlain (event) {   
        ...
        hd='As on ' + todated        
        var RefNo='PROC|' + constants.getprocedurename('SP_GetBS',constants.COMPANY) +   ';@dtTo|\'' + todate + '\'';
        var reportURL=constants.REPORTURL + '/ReportPrint.aspx?ReportCode=RPTBS_PLN&RefNo=' + RefNo + '&heading=' + hd + '&Company=' + constants.COMPANY
        window.open(reportURL,'new window', 'fullscreen=yes, scrollbars=auto, height=screen.height, left=0, top=0, width=screen.width, dependant=no, location=0, alwaysRaised=no, menubar=no, resizeable=no, scrollbars=n, toolbar=no, status=no, center=yes')        
    }
    resetOptions (event){}
    ...
    viewReport (event) {
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/getbs',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
