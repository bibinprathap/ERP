import RPT_Purchase_Form from './RPT_Purchase_Form';
...
export default class RPT_Purchase extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_GetPurchaseReport',constants.COMPANY) +   ';@fromdates|' + yyyymmddf + ';@todates|' + yyyymmddt + ';@br|' + constants.getFieldValue( this.RPT_DailyOpOptions.branch) + ';@sman|' + constants.getFieldValue(this.RPT_DailyOpOptions.salesman) + ';@custorsup|' + constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)+ ';@trntype|' + constants.getFieldValue(this.RPT_DailyOpOptions.acname);
        var reportURL=constants.DSREPORTURL + '/ReportPrint.aspx?ReportCode=PUR_REPORT&RefNo=' + RefNo + '&heading=' + ddmmyyyyfrom  +' - ' + ddmmyyyyto + '&subheading={BRANCH}&subheading1=\'' + mSub1 + '\'&Company=' + constants.COMPANY;
    ...
    viewReport (event) {
       ...
       if(constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)) 
        ...
        if(constants.getFieldValue(this.RPT_DailyOpOptions.acname)) 
           loadrequest.parem6 =  constants.getFieldValue(this.RPT_DailyOpOptions.acname)
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/getpurchasereport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "vicno", name: "Inv. No", field: "vicno", width: 70 });
                columns.push({ id: "gross", name: "Gross", field: "gross", width: 90, formatter:numformatter,  groupTotalsFormatter: sumTotalsFormatter });
                columns.push({ id: "amount", name: "Amount", field: "amount", formatter:numformatter,  groupTotalsFormatter: sumTotalsFormatter, width: 90 } );
                columns.push({ id: "supplier", name: "Supplier", field: "ACNAME", width: 180} );               
                var columnFilters = {};
    ...
    render () {
        ...
        <div>   
            ...
            {  <RPT_Purchase_Form RPT_DailyOpOptions={this.RPT_DailyOpOptions} 
                    viewReport = {this.viewReport} printReport={this.printReport} 
                    resetOptions={this.resetOptions} reportTitle='Purchase Report'/> }
