import RPT_DatewisePurchase_Form from './RPT_DatewisePurchase_Form';
...
export default class RPT_DatewisePurchase extends React.Component 
{
    constructor (props) {
        super(props);
    ...
    printReport (event) {
        ...
        var RefNo='PROC|' + constants.getprocedurename('SP_GetDateWisePurchaseReport',constants.COMPANY) +   ';@fromdates|' + yyyymmddf + ';@todates|' + yyyymmddt + ';@br|' + constants.getFieldValue( this.RPT_DailyOpOptions.branch) + ';@sman|' + constants.getFieldValue(this.RPT_DailyOpOptions.salesman) + ';@custorsup|' + constants.getFieldValue(this.RPT_DailyOpOptions.custsupno);
        var reportURL=constants.DSREPORTURL + '/ReportPrint.aspx?ReportCode=DATE_PUR_REPORT&RefNo=' + RefNo + '&heading=' + ddmmyyyyfrom  +' - ' + ddmmyyyyto + '&subheading={BRANCH}&subheading1=\'' + mSub1 + '\'&Company=' + constants.COMPANY;
    ...
    viewReport (event) {
       ...
       if(constants.getFieldValue(this.RPT_DailyOpOptions.custsupno)) 
        ...
        window.openModal(); $.ajax({
            url: constants.SERVICEURL +'/datewisepurchasereport',
            type: 'POST',
            dataType: 'json',
            data: loadrequest,
            success: function (respoce, textStatus, xhr) {  window.closeModal();
                ...
                columns.push({ id: "amount", name: "Amount", field: "amt", formatter:numformatter,  groupTotalsFormatter: sumTotalsFormatter, width: 90 } );
                columns.push({ id: "cost", name: "Cost", field: "cost", width: 50 });
                columns.push({ id: "margin", name: "Margin", field: "margin", width: 50 } );
                columns.push({ id: "per", name: "Percentage", field: "per", width: 50} );
    ...
    render () {
        ...
        <div>   
            ...
            {  <RPT_DatewisePurchase_Form RPT_DailyOpOptions={this.RPT_DailyOpOptions} 
                    viewReport = {this.viewReport} printReport={this.printReport} 
                    resetOptions={this.resetOptions} reportTitle='Datewise Purchase Report'/> }
